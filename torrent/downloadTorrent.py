...
def get_peers(tracker_url, info_hash, length): #get req
    ...
    try:
        ...
        response.raise_for_status()  #HTTP error responses
    ...
    if isinstance(raw_peers, bytes):
        ...
        # if Not in compact format decode each peer dictionary
...
def receive_message(sock):
    ...
    try:
        ...
        #to ensure we receive the full message
        message = bytearray()
...
def handle_peer_messages(sock): #handles peer message after handshake
    while True:
...
def send_request_message(sock, index, begin, length): #to peers
    message_id = b'\x06'
    payload = (
...
def download_piece(sock, piece_index, piece_length, output_path, max_retries=3):
    ...
    while retries < max_retries:
        ...
        # All blocks for the piece successfully received
        piece_data = b''.join(blocks)
        with open(output_path, 'ab') as file:
            file.write(piece_data)
        return  # Exit if successful else repeat
    raise Exception(f"Failed to download piece {piece_index} after {max_retries} retries.")

def download_torrent(torrent_file, output_file): #download torrent file and save to disk
    ...
    for piece_index in range(len(pieces_hash)):
        ...
        try:
            ...
            continue  # Move to the next piece error arrises
        
        # integrate and verify downloaded piece
